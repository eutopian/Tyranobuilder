tyrano.plugin.kag.menu = {
    ...
    doSave: function(num) {
        ...
        if (this.snap == null) this.snapSave(this.kag.stat.current_save_str, function() {
            ...
            $.setStorage(that.kag.config.projectID + "_tyrano_data",
                array_save, that.kag.config.configSave)
        })
    },
    ...
    createImgCode: function(canvas) {
        ...
        if (q == "low") code = canvas.toDataURL("image/jpeg",
            0.3);
    ...
    screenFull: function() {
        if ($.isNWJS() == true) {
            ...
            var isEnableFullScreen = document.fullscreenEnabled ||
                document.webkitFullscreenEnabled || document.mozFullScreenEnabled || document.msFullscreenEnabled || false;
